...
class OccFlowHead(BaseModule):
    def __init__(
        ...
        if gaussian_head is not None:
            self.gaussian_render = builder.build_head(gaussian_head)
        else:
    ...
    def forward_coarse_voxel(self, voxel_feats):
        ...
        if self.with_cp and  out_voxel_feats.requires_grad:
            out_voxel = cp(self.occ_pred_conv, out_voxel_feats).permute(0, 1, 3, 2, 4)
        else:
            out_voxel = self.occ_pred_conv(out_voxel_feats).permute(0, 1, 3, 2, 4)

        if self.with_cp and  out_voxel_feats.requires_grad:
            out_flow = cp(self.flow_pred_conv, out_voxel_feats).permute(0, 1, 3, 2, 4)
        else:
            out_flow = self.flow_pred_conv(out_voxel_feats).permute(0, 1, 3, 2, 4)
    ...
    @force_fp32()
    def loss_flow(self, output_flow, target_voxels, target_flow, tag, flow_loss_weight=None):

        # resize gt

        output_flow[torch.isnan(output_flow)] = 0
        output_flow[torch.isinf(output_flow)] = 0
        assert torch.isnan(output_flow).sum().item() == 0
        assert torch.isnan(target_voxels).sum().item() == 0

        output_flow = output_flow.permute(0, 2, 3, 4, 1)
        # output_voxels = torch.log(output_voxels * 0) + output_voxels/0 # debug !!!!!!!!

        if self.use_flow_mask:
            move_object_mask = target_voxels <= self.move_idx
            static_object_mask = (target_voxels > self.move_idx) & ((target_voxels < self.empty_idx))
            print(output_flow.shape, target_flow.shape)
            #print(move_object_mask.shape, static_object_mask.shape, move_object_mask.dtype, print(output_flow.shape))


        loss_dict = {}

        # igore 255 = ignore noise. we keep the loss backward for the label=0 (free voxels)

        if self.use_flow_weight:
            norm_weight = torch.norm(target_flow, dim=-1)[..., None] + 0.1
            #move_object_mask = target_voxels <= self.move_idx
            #norm_weight = move_object_mask.float()[..., None]
        ...
        loss_dict['loss_flow{}'.format(tag)] = self.flow_loss(output_flow, target_flow,
                                                                  weight=norm_weight, avg_factor=avg_factor) * flow_loss_weight


        return loss_dict

    @force_fp32() 
    def loss(self, output_voxels=None, output_flows=None,
                output_coords_fine=None, output_voxels_fine=None, 
                target_voxels=None, target_flow=None, target_gaussian=None, flow_loss_weight=None,
             visible_mask=None, output_render=None, **kwargs):
        ...
        if self.use_3d_loss:
            for index, output_voxel in enumerate(output_voxels):
                ...
                #loss_dict.update(self.gaussian_render.loss_3d(output_voxel, target_voxels))
            for index, output_flow in enumerate(output_flows):
                loss_dict.update(self.loss_flow(output_flow, target_voxels, target_flow, tag='c_{}'.format(index),
                                                flow_loss_weight=flow_loss_weight))


        if output_render is not None:
            for index, render_result in enumerate(output_render):
                loss_dict.update(self.gaussian_render.loss_2d(render_result, target_gaussian,
                                                flow_loss_weight=flow_loss_weight, move_idx=self.move_idx))
