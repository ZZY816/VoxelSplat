...
class Feats2Gau_gt(nn.Module):
    ...
    def forward(self, pts, sem_gt, flow_gt):
        ret = {}
        ret['xyz'] = pts
        ret['xyzt'] = torch.cat(((pts[..., :2] + flow_gt), pts[..., -1][..., None]), dim=-1)
        ret['shs'] = torch.cat((sem_gt, flow_gt), dim=1)
...
class Feats2Gau(nn.Module):
    ...
    def forward(self, pts, semantics, flows, gau_feat=None):
        ret = {}
        ret['xyz'] = pts
        ret['xyzt'] = torch.cat(((pts[..., :2] + flows), pts[..., -1][..., None]), dim=-1)
        ret['shs'] = torch.cat((semantics[..., :-1], flows), dim=1)
        if gau_feat is not None:
            ret['opacity'] = torch.sigmoid(gau_feat[..., -1])[..., None]
        else:
            ret['opacity'] = torch.sigmoid(-semantics[..., -1])[..., None]
            #ret['opacity'] = torch.clamp(1-semantics[..., -1], min=0.0, max=1.0)[..., None]
        # ret['opacity'] = (2 / (torch.exp(feature[..., 0] * 2) + torch.exp(-feature[..., 0] * 2)))[..., None]
        ret['scaling'] = self.scaling.repeat(pts.size(0), 1)
        ret['rotation'] = self.rotation.repeat(pts.size(0), 1)
...
class GaussianHead_Gsplat(nn.Module):
    def __init__(self,
        ...
        #self.feat2gau = Feats2Gau_v5(gaussian_cfg=gaussian_cfg)
        self.feat2gau = Feats2Gau(gaussian_cfg)
        self.feat2gau_gt = Feats2Gau_gt(gaussian_cfg)
    ...
    def forward_single_batch(self, semantics, flows, c2ws, intrinsics, gt_3d=None, gt_flow=None, sample_mask=None):

        pts, semantics, flows, sem_gt, pts_gt, flow_gt = self.sample_feature(semantics, flows, gt_3d, gt_flow, sample_mask)

        gs = self.feat2gau(pts, semantics, flows)

        gs_gt = self.feat2gau_gt(pts_gt, sem_gt, flow_gt)

        sem_pred, flow_pred, depth_pred = self.forward_rendering(gs, c2ws, intrinsics)
        sem_gt, flow_gt, depth_gt = self.forward_rendering(gs_gt, c2ws, intrinsics)

        return sem_pred, flow_pred, depth_pred, sem_gt, flow_gt, depth_gt
    ...
    def loss_2d(self, render_result, gaussian_label, flow_loss_weight=None, move_idx=None):
        ...
        #c2w, intrinsic, label_segs, label_depths, label_flows = gaussian_label
        #vel = torch.sqrt(label_flows[..., 0] ** 2 + label_flows[..., 1] ** 2).mean()
        ...
        #fore_mask = (depth_gt > 0.1) & (semantics_gt.sum(-1) > 0.1)
        fore_mask = (depth_gt > 0.1)
        #print(semantics_gt.shape, fore_mask.shape, semantics_gt[fore_mask].shape)
        #loss_sem = self.loss_sem(semantics[fore_mask], semantics_gt[fore_mask].argmax(-1)) * self.loss_sem_weight
        loss_sem = self.loss_sem(semantics[fore_mask], semantics_gt[fore_mask]) * self.loss_sem_weight

        # fore_mask2 = (depth_gt > 0.1)
        # loss_sem = self.loss_sem(semantics[fore_mask2], semantics_gt[fore_mask2]) * self.loss_sem_weight
    ...
    def forward(self, semantics, flows, gaussian_label, gt_3d=None, gt_flow=None):
        c2w, intrinsic, sample_mask = gaussian_label

        # depth_save = label_depths[0, 0, 0].detach().cpu().numpy()
        # depth_save.tofile('/zhuziyue/zzy/GaussianOcc/visual/depth_map_gt.bin')
        ...
        for b in range(batch_size):
            ...
            sem_2d, flow_2d, depth_2d, sem_2d_gt, flow_2d_gt, depth_2d_gt = self.forward_single_batch(semantics[b],
                    flows[b], c2w[b], intrinsic[b], gt_3d=gt_3d[b], gt_flow=gt_flow[b], sample_mask=sample_mask[b])
