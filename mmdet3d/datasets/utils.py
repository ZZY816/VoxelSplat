...
def downsample_intrinsic(K, downsampling_factor=4):
    """
    Adjust a batch of camera intrinsic matrices for an image that has been downsampled by a specified factor.
    Using PyTorch for batch processing.

    Parameters:
    K (torch.Tensor): The batch of camera intrinsic matrices with shape (T, N, 3, 3).
    downsampling_factor (int): The factor by which the image resolution has been reduced.

    Returns:
    torch.Tensor: The batch of modified camera intrinsic matrices.
    """
    # Convert K to float for division
    K_new = K.float()

    # Adjust the focal lengths and the principal points
    K_new[..., 0, 0] /= downsampling_factor  # fx
    K_new[..., 1, 1] /= downsampling_factor  # fy
    K_new[..., 0, 2] /= downsampling_factor  # cx
    K_new[..., 1, 2] /= downsampling_factor  # cy
