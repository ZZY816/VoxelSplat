...
class Metric_mIoU_SurroundOcc():
    ...
    def add_batch(self, semantics_pred, semantics_gt):
        ...
        masked_semantics_pred[masked_semantics_pred != self.num_classes - 1] = 0
        masked_semantics_pred[masked_semantics_pred == self.num_classes - 1] = 1

        masked_semantics_gt[masked_semantics_gt != self.num_classes - 1] = 0
        masked_semantics_gt[masked_semantics_gt == self.num_classes - 1] = 1
    ...
    def count_miou(self):
        res = {}
        mIoU = self.per_class_iu(self.hist)
        IoU = self.per_class_iu(self.hist_iou)
        # assert cnt == num_samples, 'some samples are not included in the miou calculation'
        print(f'===> per class IoU of {self.cnt} samples:')
        for ind_class in range(self.num_classes - 1):
            print(f'===> {self.class_names[ind_class]} - IoU = ' + str(round(mIoU[ind_class] * 100, 4)))
        ...
        res['Overall'] = round(np.nanmean(mIoU[:self.num_classes - 1]) * 100, 2)
        # print(f'===> sample-wise averaged mIoU of {cnt} samples: ' + str(round(np.nanmean(mIoU_avg), 2)))
        print(f'===> IoU of {self.cnt} samples: ' + str(round(IoU[0] * 100, 4)))
