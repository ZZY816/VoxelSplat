...
VIZ = False
dvr = load("dvr", sources=["ray_iou/lib/dvr/dvr.cpp", "ray_iou/lib/dvr/dvr.cu"], verbose=True,
...
def process_one_sample(sem_pred, lidar_rays, output_origin, flow_pred, return_xyz=False):
    ...
    for t in range(T):
        ...
        if return_xyz:
            pred_pcds = torch.cat([pred_label, pred_dist, pred_flow, pred_pcds[0]],
                                  dim=-1)  # [N, 5]  5: [label, dist, x, y, z]
        else:
...
def quick_save(results_dict, save_dir):
    ...
    for batch in tqdm(data_loader, ncols=50):
        ...
        pred_data = results_dict[sample_token]

        sem_pred = pred_data['semantics']
        sem_pred = np.reshape(sem_pred, [200, 200, 16])
        sem_pred = torch.from_numpy(sem_pred)

        flow_pred = np.array(pred_data['flow'])
        #print(flow_pred.shape)
        flow_map = np.zeros((200, 200, 16, 2))

        if flow_pred.shape != (200, 200, 16, 2):
            pos = sem_pred <= 7
            if pos.sum() > 0:
                flow_map[pos] = flow_pred
        flow_pred = flow_map
        # print(flow_pred.shape, 'dadaw')
        # exit()
    ...
    output_path_pred = os.path.join(save_dir, output_name)
...
def main(args):
    ...
    for info in data_infos:
        # get reletive path'
        token2path[info['token']] = os.path.join(args.pred_root, info['scene_name'], info['token'], args.out_type)
    ...
    for batch in tqdm(data_loader, ncols=50):
        ...
        pred_filepath = token2path[sample_token]
    ...
    output_path_pred = os.path.join(args.output_dir, args.output_name)
    print("gzip and dumping the data...")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-info", default='/zhuziyue/zzy/GaussianOcc/data/nuscenes/bevdetv2-nuscenes_infos_val.pkl')
    parser.add_argument("--pred-root", default='/zhuziyue/zzy/GaussianOcc/test/fbocc-r50-gau_20/Sun_Sep_22_04_20')
    parser.add_argument("--output-dir", default='/zhuziyue/zzy/GaussianOcc/test/fbocc-r50-gau_20')
    parser.add_argument("--output-name", default='Sun_Sep_22_04_20.gz')
    parser.add_argument("--out-type", default='preds.npz')
