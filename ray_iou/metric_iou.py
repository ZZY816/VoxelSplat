...
def convert2occ(occ, class_num=16):
    occ[occ<class_num] = 0
    occ[occ==class_num] = 1
    return occ
...
def compute(args):
    ...
    for gt_token in tqdm(openocc_test_file['results'].keys()):
        ...
        if gt_token in pred_file['results'].keys():  # found
            ...
            # print(pred_data['pcd_cls'].shape, pred_data['pcd_cls'].min(), pred_data['pcd_cls'].max())
            pred_data['pcd_cls'] = convert2occ(pred_data['pcd_cls'])
            gt_data['pcd_cls'] = convert2occ(gt_data['pcd_cls'])
            # print(pred_data['pcd_cls'].shape, pred_data['pcd_cls'].min(), pred_data['pcd_cls'].max())
            #
            # exit()
