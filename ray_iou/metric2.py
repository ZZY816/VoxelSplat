...
def calc_metrics(pred_cls_list, pred_dist_list, pred_flow_list, gt_cls_list, gt_dist_list, gt_flow_list):
    ...
    for idx in range(len(pred_cls_list)):
...
def compute(args):
    ...
    index_list = []
    scene_name_list = []
    for i in tqdm(range(len(gt_token_list))):
        openocc_iou_list, openocc_ave_list = calc_metrics(pred_cls_list[i], pred_dist_list[i], pred_flow_list[i],
                                                          gt_cls_list[i], gt_dist_list[i], gt_flow_list[i])
        ...
        ave_list.append(openocc_mave)
        iou_list.append(openocc_miou)
        index_list.append(i)
        scene_name_list.append(data_infos[i]['scene_name'])

    if not (len(gt_token_list) == len(ave_list) == len(iou_list)):
    ...
    with open(csv_file_name, mode='w', newline='', encoding='utf-8') as csv_file:
        ...
        # 写入表头（可选）
        csv_writer.writerow(['index', 'scene_name', 'token', 'iou', 'ave' ])

        # 写入数据行
        for gt_token, scene_name, openocc_ave, openocc_iou, index in zip(gt_token_list, scene_name_list, ave_list, iou_list, index_list):
            csv_writer.writerow([index, scene_name, gt_token, openocc_iou, openocc_ave])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--csv", default='rank')
